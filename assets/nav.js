/* =============================================================================
   ...
   Reads NAV from nav-data.js and builds it into <div id="site-nav">.
   No dependencies. Drop both files, nav.css and the div into any wiki page.
   ========================================================================== */
(function () {
  "use strict";
  ...
  function build(root) {
    const brandHref = root.dataset.home || "#";
    const logo = root.dataset.logo || "assets/logo.png";
    ...
    NAV.forEach((tab) => {
      ...
      /* student artwork, if it has been dropped into assets/tab-icons/ */
      ...
      tab.pages.forEach((p) => list.appendChild(entry(p)));
      ...
      tab.pages.forEach((p) => gbody.appendChild(entry(p)));
  ...
  function entry(p) {
    const a = el("a", "sitenav__entry" + (p.current ? " is-current" : ""));
    a.href = p.href;
