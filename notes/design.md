...
```
index.html                the page: nav mount, group shot, section nav, profile view
assets/data/site-nav.js   the five tabs and their sub-pages
assets/data/roster.js     the roster. This is the file you edit.
assets/css/nav.css        nav styling
assets/css/team.css       page styling
assets/js/nav.js          nav renderer, 26 icons, open and close behaviour
assets/js/team.js         card renderer, scroll-spy, profile view
assets/img/logo.png       team badge at 200px
assets/img/group.jpg      group shot at 2400px, 624 KB
assets/img/members/       45 portraits, cropped 4:5 at 720x900, about 70 KB each
assets/img/tab-icons/     drop the student drawings here (see the README inside)
assets/fonts/             inter-variable.ttf, inlined into the bundle
build/bundle.py           folds it all into one file
```

Open `index.html` in a browser. No build step, no dependencies, nothing loaded
...
from `LABEL_GRADIENTS` in `assets/data/roster.js`, so promoting another label later is a
one-line change.
...
`assets/data/roster.js`. Every card picks it up automatically.
...
`assets/data/roster.js` rather than sorting them. Instructors keep the order you set,
with Dr. Pak last.
...
held one, which is the point. Set `openSlots` on the section in `assets/data/roster.js`
...
SVG in `assets/js/nav.js`, so nothing is fetched from outside.

**Tab artwork.** Each tab looks for `assets/img/tab-icons/<id>.png`, so
...
guidance is in `assets/img/tab-icons/README.txt`.

**To put the nav on another page**, add three things:

```html
<link rel="stylesheet" href="assets/css/nav.css" />
<div id="site-nav" data-tab="wetlab" data-home="index.html" data-logo="assets/img/logo.png"></div>
<div class="sitenav-spacer"></div>
...
<script src="assets/data/site-nav.js"></script>
...
`assets/data/site-nav.js`. Every `href` is a `#` placeholder except `index.html`, so fill
...
Everything is in `SECTIONS` in `assets/data/roster.js`:

```js
{ name: "Abby Tsai", role: "Wet Lab Lead",
  photo: "assets/img/members/abby-tsai.jpg",
  funPhoto: "assets/img/members/abby-tsai-fun.jpg",   // optional, shown on hover
...
```bash
magick "_source/photos/NAME.jpg" -auto-orient -resize 720x900^ -gravity north -extent 720x900 -strip -quality 82 assets/img/members/name.jpg
```
...
1. **The "Minor" definition is my wording.** You defined the three major tracks
   ...
   Rewrite it in `legend()` in `assets/js/team.js` if you want it put differently. It is
...
4. **Ian Cheng is in.** He was not on your list of nine advisors, but you
   ...
   Delete his block from `assets/data/roster.js` if that was a stray upload.
...
The wiki blocks resources from other servers. `assets/css/team.css` currently pulls Inter
