...
`share/public/index.html` is the whole page in one file. Every photo, the logo,
...
3. On the repo page click **Add file → Upload files**, drag in
   `share/public/index.html`, and commit.
...
1. Sign in at **dash.cloudflare.com**, then **Workers & Pages → Create →
   Pages → Upload assets**.
2. Name the project, drag in the `share/public` folder, and deploy.
...
2. Drag the `share/public` folder onto the page.
...
**The page is set to noindex.** `share/public/index.html` carries
...
Then re-upload `share/public/index.html`. The bundler re-reads every file under
